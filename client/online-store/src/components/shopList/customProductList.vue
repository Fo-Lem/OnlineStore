...
<template lang="">
  <div class="bg-white">
    <div class="mx-auto max-w-2xl px-4 pt-3 pb-6 flex flex-col items-center gap-6 sm:px-6  lg:max-w-7xl lg:px-8">
      ...
      >
        <router-link
          v-for="type in catalog.categories[$route.params.categoryId].product_types"
          :key="type.id"
          :basket="basket"
          :to="{ name: 'productOverviews', params: { productId: type.id, heroId: searchFirstHero($route.params.categoryId, type.id) } }"
        >
          <custom-list
            v-if="type.name != '-' && type.items "
            :list="type"
          />
        </router-link>
      </div>
